...
Make sure to set `testplot = TRUE` below if you have no ggplot objects in your current global environment; `memeApp` will create a testplot.
This can be left as `FALSE` if you have your own ggplot objects you wish to use.

```{r app, eval=FALSE}
library(memery)
memeApp(testplot = TRUE) # or memeApp()
```

### ImageMagick suggested but not required

If the `magick` package and ImageMagick software are installed on the system, `memeApp` will launch in complete mode and the default image will be an animated gif.
If not installed, a simplified version of the example app will launch, and only jpg and png files can be used.
Some users have no interest in animated gifs and may not have ImageMagick installed on their operating system.
This software is not required since working with animated gifs represent only one small aspect of `memery`.
...
```{r sources_and_outputs1, eval=FALSE}
loc <- system.file("philosoraptor.jpg", package = "memery")
